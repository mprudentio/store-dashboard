...
import { getRevenue, postRevenue, editRevenue, deleteRevenue } from "./revenueSlice"
import { getProducts } from "../Stocks/productSlice"
import dayjs from 'dayjs'
import Swal from "sweetalert2"
import { Table,DatePicker, Button, Modal, Input, Form,AutoComplete,InputNumber } from "antd"
 ...
 const columns = [
    {
      ...
      render:(products)=>(
        <div>
          {products.map((e) => <p key={e.name+1}>{e.name}</p>)}
        </div>
      ),
      // sorter: (a, b) => {
      ...
      render:(products)=>(
        <div>
          {products.map((e,i) => <p key={`${e.name}-${e.date}-${i}-${e.price}`}>{e.quantity}</p>)}
        </div>
      )
    },{
      title:'Price',
...
const handleDeletProduct =(record) =>{
  Swal.fire({
    title: 'Are you sure?',
...
const addProductItem = () => {
  ...
  form.validateFields([`product-${productItems.length - 1}`, `quantity-${productItems.length - 1}`]).then(() => {
    const newProductItems = [...productItems, { product: '', quantity: 0, price: 0 }];
    setProductItems(newProductItems);
    form.resetFields([`product-${newProductItems.length - 1}`, `quantity-${newProductItems.length - 1}`, `price-${newProductItems.length - 1}`]);
  });
};
  ...
  const calculatePrice = (quantity, productName) => {
   ...
   const handleProductChange = (value, index) => {
    ...
    updatedProductItems[index] = { ...updatedProductItems[index], product: productName };
  ...
  const onFinish = (values) => {
    ...
    for (const item of productItems) {
      const matchingProduct = dataProducts.products.find(product => product.name === item.product);
      if (matchingProduct) {
  ...
  const isProductNameUnique = (currentProductName, productItems) => {
    ...
    const duplicateCount = productNames.filter((name) => name === currentProductName).length;
  ...
  return (
    <div className="p-4">
      ...
      <Modal  title={<p className="text-lg">{editData ? "Edit Sales" : "Add Sales"}</p>}
            open={modalCreate}
            centered
          ...
          <Form layout="vertical" form={form} onFinish={(values)=>onFinish(values)}>
              ...
              <div key={index} className="flex gap-2">
                <Form.Item name={`product-${index}`} label="Product" className="w-full" rules={[
                      {
                        required: true,
                  ...
                  <AutoComplete
                    ...
                    allowClear={false}
                    options={dataProducts.products.map(product => ({ value: product.name }))}
                    filterOption={true}
                    className="w-full"
                ...
                >
                  <InputNumber
                    ...
                    onChange={(value) => {
                      ...
                      const updatedProductItems = [...productItems];
                      updatedProductItems[index] = { ...updatedProductItems[index], quantity: value};
                      setProductItems(updatedProductItems);
          ...
          <Form.Item name="totalPrice" label="Total Price">
            <Input value={0} disabled/>
             ...
             <Form.Item>
              <Button onClick={()=>addProductItem()}>Add new Products</Button>
             </Form.Item>    
            <Form.Item>
              <Button htmlType="submit" type='primary' className='bg-primary-green py-6 font-poppins flex items-center justify-center'>Submit</Button>
            </Form.Item>
          </Form>
      </Modal>
    </div>
