  ...
  async (uid, { rejectWithValue }) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_KEY}/sales?uid=${uid}`
      );
...
// POST
export const postRevenue = createAsyncThunk(
  "postRevenue",
  async ({ date, products, price, uid }, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_KEY}/sales`, {
        ...
        body: JSON.stringify({ date, price, products: productsWithoutId, uid }),
      });
...
export const editRevenue = createAsyncThunk(
  "editRevenue",
  async ({ id, date, products, price, uid }, { rejectWithValue }) => {
    try {
      const productsWithoutId = products.map((product) => {
        // eslint-disable-next-line no-unused-vars
        const { id, ...rest } = product;
      ...
      //   console.log(productsWithoutId)
      const response = await fetch(
        `${import.meta.env.VITE_API_KEY}/sales/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteRevenue.fulfilled, (state, action) => {
      ...
      state.revenue = state.revenue.filter(
        (product) => product.id !== deleteRevenueId
      );
    });
